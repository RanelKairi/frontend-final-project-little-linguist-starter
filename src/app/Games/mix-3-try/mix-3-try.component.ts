...
export class Mix3TryComponent implements OnInit {
  displayedColumns: string[] = ['words', 'wordResults'];
  ...
  nextWord() {
    ...
    if (this.words && this.index < this.words.length - 1) {
      console.log(
        'nextWord()=>inside=>if(this.words && this.index < this.words.length - 1){'
      );
      this.index++;
      ...
      this.mixWord = [...this.words[this.index].origin]
        ...
        .toLocaleUpperCase();
      if (
        this.mixWord.toLocaleLowerCase() ===
  ...
  submit() {
    ...
    if (!isEndOfGame) console.log('!isEndOfGame', !isEndOfGame, isEndOfGame);
